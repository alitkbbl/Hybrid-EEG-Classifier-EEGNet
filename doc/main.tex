...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{../figures/eegnet_accuracy_per_subject.png}
\caption{LOSO test accuracy per subject for EEGNet, with reference lines for the CSP+MLP LOSO
baseline mean ($40\%$) and chance level ($25\%$).}
\label{fig:exp3-accuracy}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.55\linewidth]{../figures/eegnet_confusion_matrix_pooled.png}
\caption{Aggregated (row-normalized) confusion matrix across all nine EEGNet LOSO folds.}
\label{fig:exp3-confmat}
\end{figure}
...
artifact) rather than an artifact of either particular modelling approach.
...
epochs at a reduced learning rate, then evaluated on the remaining $50\%$ of that subject's data.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{../figures/eegnet_finetune_vs_zero_shot.png}
\caption{Zero-calibration (LOSO) vs.\ subject-specific fine-tuned accuracy for EEGNet, per subject.}
\label{fig:exp3-finetune}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{../figures/accuracy_comparison_all_approaches.png}
\caption{Comparison of mean cross-subject accuracy across the three experimental approaches:
subject-dependent baseline (mean over unseen subjects), LOSO Hybrid WPD-CSP-MLP, and LOSO EEGNet
(zero-shot and fine-tuned), with the chance-level reference line.}
\label{fig:overall-comparison}
\end{figure}

The results in Table~\ref{tab:overall-comparison} and Figure~\ref{fig:overall-comparison} tell a
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{../figures/wpd_energy_features.png}
\caption{Distribution of the WPD energy feature (sensorimotor ROI average) across the four
motor-imagery classes and the three selected sub-bands, Subject A01T.}
\label{fig:wpd-energy-exp1}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{../figures/per_class_f1_across_subjects.png}
\caption{Spread of per-class F1-scores across the nine LOSO folds (Hybrid WPD-CSP-MLP).}
...
\subsection{Experiment 3 -- Supplementary Figures}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{../figures/eegnet_training_curves_grid.png}
\caption{Training vs.\ validation accuracy curves for each of the nine EEGNet LOSO folds.}
\label{fig:eegnet-training-grid}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{"../figures/ee"}
\caption{Per-class F1-score distribution across the nine EEGNet LOSO folds.}
\label{fig:eegnet-f1-boxplot}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{../figures/final_loso_test_summary.png}
\caption{Final EEGNet LOSO summary: per-subject accuracy relative to the CSP+MLP baseline and chance
level, with the overall mean and $\pm 1$ SD band.}
\label{fig:eegnet-final-summary}
\end{figure}
